...
def main(cfg):
    ...
    accelerator.init_trackers(project_name=None)
        ...
        for step, batch in enumerate(torch_format_dataloader):
            ...
            if cfg.resume_from_checkpoint and epoch == starting_epoch:
                if resume_step is not None and step < resume_step:
                    if step % gradient_accumulation_steps == 0:
                        ...
                        completed_steps += 1
                    continue
            if cfg.forget_loss.endswith("pd"):
                forget_inputs, retain_inputs, pd_input = batch
            else:
            ...
            with accelerator.accumulate(model):
                if cfg.forget_loss == "ga":
                    ...
                    loss = outputs.loss
                    loss = loss * -1
                    forget_losses.append(loss.detach().item())
                elif cfg.forget_loss == "gapd":
                    outputs = model(**forget_inputs)
                    loss = outputs.loss
                    loss = loss * -1
                    pd_outputs = model(**pd_input)
                    pd_loss = pd_outputs.loss
                    
                    loss =  loss + 0.5*pd_loss
                    forget_losses.append(loss.detach().item())
                    pd_losses.append(pd_loss.detach().item())
                
                elif cfg.forget_loss == "gdpd":
                    outputs = model(**forget_inputs)
                    ...
                    loss = 1.0*forget_loss + 0.5*retain_loss + 0.5*pd_loss
                    forget_losses.append(forget_loss.detach().item())
                    retain_losses.append(retain_loss.detach().item())
                    pd_losses.append(pd_loss.detach().item())
    ...
    if accelerator.is_main_process:
        ...
        if any([
            ...
            # forget loss
            if len(forget_losses) > 0:
                plt.plot(
                    range(len(forget_losses)),
                    forget_losses,
                    label="Forget Loss"
                )
            ...
            # 总体（losses）
            if len(losses) > 0:
                float_losses = [x.item() if isinstance(x, torch.Tensor) else x for x in losses]
                plt.plot(
                    range(len(float_losses)),
                    float_losses,
                    label="Total Loss"
                )
            ...
            # KL loss
            if len(kl_losses) > 0:
                float_kl_losses = [x.item() if isinstance(x, torch.Tensor) else x for x in kl_losses]
                plt.plot(
                    range(len(float_kl_losses)),
                    float_kl_losses,
                    label="KL Loss"
                )
            ...
            loss_plot_path = os.path.join(cfg.save_dir, "loss_curves.png")
            plt.savefig(loss_plot_path)
            plt.close()
            print(f"Loss curves have been saved to {loss_plot_path}")
